...
def LoginPage(requset):
    ...
    if requset.method == 'POST':
        ...
        try:
            ...
            messages.error(requset , 'Sorry User Does Not Exit')
        ...
        if user is not None:
           ...
           messages.error(requset , 'Sorry User Does Not Exit 2')
...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        rooms = Room.objects.filter(grade=user_grade)
...
def createRoom(request):
    ...
    if request.method == 'POST':
        ...
        # if form.is_valid():
        #     # we use commit = false toi create an instance or variable of the room model 
        #     room = form.save(commit=False)
        #     # then we go to the room intance , get the host and set it to request.user which is the loged in user
